...
import { discoverGlobalDataFiles } from '../utils/configDiscovery';
...
const PANEL_ID = 'data_browser_global';
...
function createEditorConfig(
  ...
  const icon = metadata ? getFileIcon(metadata, loadGlobalData(fileKey, null)) : '📄';
  ...
  return {
    ...
    infoLines,
    getData: () => loadGlobalData(fileKey, null),
    saveData: (data) => { saveGlobalData(fileKey, data); },
    template: metadata?.template,
    fileExists: metadata?.exists ?? (loadGlobalData(fileKey, null) !== null),
    accentColor: 0xE74C3C,
    emptyAccentColor: 0x95A5A6,
    ...
    extraButtons: [
      {
        ...
        disabled: !(metadata?.exists ?? (loadGlobalData(fileKey, null) !== null)),
        row: 2,
      },
    ],
  };
...
const dataBrowserGlobalPanel: PanelOptions = {
  id: PANEL_ID,
  name: 'Global Data Browser',
  description: 'Browse and edit global data files (all guilds)',
  category: 'Advanced',
  panelScope: 'system',
  devOnly: true,
  ...
  callback: async (context: PanelContext): Promise<PanelResponse> => {
    ...
    if (panelData?.selectedFile) {
      const dataFiles = discoverGlobalDataFiles();
      const metadata = dataFiles.find(f => `${f.moduleName}/${f.id}` === panelData.selectedFile);
      const config = createEditorConfig(panelData.selectedFile, metadata);
  ...
  handleButton: async (context: PanelContext, buttonId: string): Promise<PanelResponse | null> => {
    ...
    for (const action of jsonActions) {
      if (buttonId.startsWith(`${action}_`)) {
        ...
        const config = createEditorConfig(fileKey, metadata);
...
async function showModuleList(context: PanelContext, page: number): Promise<PanelResponse> {
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          '## Global Data Browser\n\n' +
          '**No global data files found.**\n\n' +
          'No modules have defined data schemas for global data yet.'
        )
      );
    return createV2Response([container]);
  }
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent('## Global Data Browser')
  );

  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent('**Warning:** Global data affects all guilds')
  );
  ...
  for (let i = 0; i < pageModules.length; i++) {
    ...
    const existingCount = moduleFiles.filter(f => f.exists).length;
    ...
    const previewText = previewItems.map(f => {
      const data = f.exists ? loadGlobalData(`${f.moduleName}/${f.id}`, null) : null;
      ...
      new SectionBuilder()
        ...
        .setButtonAccessory(
          new ButtonBuilder()
            ...
            .setLabel(`Open (${existingCount}/${fileCount})`)
...
async function showModuleView(context: PanelContext, moduleName: string, page: number): Promise<PanelResponse> {
  const allFiles = discoverGlobalDataFiles();
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent('**Scope:** Global (all guilds)')
  );
  ...
  for (const file of pageFiles) {
    ...
    const data = file.exists ? loadGlobalData(fileKey, null) : null;
    const statusIcon = getFileIcon(file, data);
    const hasData = !isDataEmpty(data);
...
async function showDeleteModal(context: PanelContext, fileKey: string): Promise<PanelResponse> {
  ...
  const confirmInput = new TextInputBuilder()
    ...
    .setLabel(`Type DELETE to confirm (affects ALL guilds)`)
...
export default dataBrowserGlobalPanel;
